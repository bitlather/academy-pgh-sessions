const partF = (whichTest) => {
  ...
  clear();
  log(`STEP F: Return the sums as a 2d array CORRECTLY.`);
  log(`--------------------------------------------------------------------------------\n`);
  log(`There we go! :-)`);
  log(``);
  log(`... hold up there cowgirl, not quite yet. Our solution is returning duplicate`);
  log(`sets! Good thing we've got those unit tests. Can you remove the duplicates?`);
  log(``);
  log(`--------------------------------------------------------------------------------\n`);
